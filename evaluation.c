...
{
        played_board *new_board;

        new_board = (played_board*)malloc(sizeof(played_board));

        for(unsigned int i=0;i<28;i++)
        {
                new_board->board[i].nb_checkers = board[i].nb_checkers;
                new_board->board[i].player = board[i].player;
        }

        new_board->next = liste;
